        ...
        if (IS_DEV) {
            ...
            serverProcess = fork(serverPath, [], {
                env: {
                    ...
                    // Sharp native binaries live in app.asar.unpacked, not inside app.asar
                    NODE_PATH: path.join(__dirname, '..', 'node_modules') + path.delimiter + path.join(__dirname, '..').replace('app.asar', 'app.asar.unpacked') + path.sep + 'node_modules'
                },
...
function createWindow() {
    ...
    mainWindow = new BrowserWindow({
        ...
        minHeight: 700,
        title: 'Alter',
        // macOS-only: sleek inset title bar with custom traffic light position
        ...(isMac ? {
            titleBarStyle: 'hiddenInset',
            trafficLightPosition: { x: 15, y: 15 },
        } : {
            // Windows/Linux: standard frame with native controls
            frame: true,
        }),
...
app.on('before-quit', () => {
    // Kill the server process cleanly (cross-platform)
    if (serverProcess) {
        try {
            // On Windows, SIGTERM doesn't work reliably on child processes.
            // process.kill() with no signal sends SIGTERM on Unix and terminates on Windows.
