...
const GalleryPanel = () => {
    ...
    const handleDownload = (img) => {
        ...
        a.download = `alter_${img.modelName || 'gen'}_${new Date(img.timestamp).toISOString().slice(0, 10)}.${ext}`;
        a.click();
    };
        ...
        return (
            <div className="h-full flex flex-col overflow-hidden">
                ...
                <div className="shrink-0 flex items-center justify-between px-3 py-2 border-b border-zinc-800/40">
                    ...
                    <div className="flex items-center gap-1.5">
                        <button
                            ...
                            className={`text-[14px] px-1.5 py-0.5 rounded transition-colors ${selectedItem.starred ? 'text-teal-400' : 'text-zinc-600 hover:text-teal-400'}`}
                        >
                ...
                <div className="flex-1 overflow-auto custom-scrollbar p-3 flex flex-col gap-3">
                    <div className="flex-1 min-h-0 flex items-center justify-center bg-[#0a0a0c] rounded-xl border border-zinc-800/40 overflow-hidden relative">
                        <img
                    ...
                    <div className="shrink-0 grid grid-cols-2 md:grid-cols-4 gap-2 text-[10px]">
                        ...
                        <div className="bg-zinc-900/80 rounded-lg px-2.5 py-1.5 border border-zinc-800/40">
                            ...
                            <span className="text-teal-400 font-mono font-medium">{selectedItem.seed || '—'}</span>
                        </div>
                        <div className="bg-zinc-900/80 rounded-lg px-2.5 py-1.5 border border-zinc-800/40">
                ...
                {isZoomed && (
                    <div
                        className="fixed inset-0 z-50 bg-[#0e0e10]/95 backdrop-blur-sm flex items-center justify-center cursor-zoom-out animate-fade-in"
                    ...
                    >
                        ...
                        />
                        <button className="absolute top-4 right-4 bg-white/10 hover:bg-white/20 text-white rounded-full p-2 transition-colors">
                ...
                <div className="shrink-0 flex items-center justify-between px-3 py-2 border-b border-zinc-800/40">
                    ...
                    <div className="flex items-center gap-1.5">
                        <button
                            ...
                            className={`text-[11px] px-2 py-0.5 rounded-md transition-colors ${filter === 'starred' ? 'text-teal-400 bg-teal-500/10' : 'text-zinc-600 hover:text-zinc-300'}`}
                        >
                            Favoris
                        </button>
                        {pagination.total > 0 && (
                                    ...
                                    >
                                        ...
                                        <div className="absolute top-1 right-1 flex gap-0.5 opacity-0 group-hover:opacity-100 transition-opacity">
                                            <button
                                                ...
                                                className={`w-5 h-5 rounded flex items-center justify-center text-[10px] transition-colors ${img.starred ? 'text-teal-400 bg-black/50' : 'text-white/60 bg-black/40 hover:text-teal-400'}`}
                                            >
                                                {img.starred ? '★' : '☆'}
                                            </button>
                                            <button
                                        ...
                                        {/* Starred indicator */}
                                        {img.starred && (
                                            <div className="absolute top-1 left-1 text-[10px] text-teal-400 group-hover:opacity-0 transition-opacity">★</div>
                                        )}
                                    </div>
                                ))}
                            </div>
