...
async function loadProfiles() {
  try { const r = await window.storage.get("model-profiles"); return r ? JSON.parse(r.value) : {}; } catch { return {}; }
}
async function saveProfiles(profiles) {
  try { await window.storage.set("model-profiles", JSON.stringify(profiles)); } catch (e) { console.error(e); }
}
async function loadHistory() {
  try { const r = await window.storage.get("gen-history"); return r ? JSON.parse(r.value) : []; } catch { return []; }
}
async function saveHistory(h) {
  try { await window.storage.set("gen-history", JSON.stringify(h.slice(0, 50))); } catch (e) { console.error(e); }
}
