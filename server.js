...
const processForStealth = async (imgBuffer) => {
    ...
    const resized = await sharp(imgBuffer)
        ...
        .resize({
            ...
            withoutEnlargement: false // Always upscale to guarantee exact output dimensions
        })
        .toBuffer({ resolveWithObject: true });

    const { width: actualW, height: actualH } = resized.info;

    // 2. Safe micro-crop: guarantee extract stays within bounds
    const cropMargin = 4;
    const safeW = Math.max(actualW - cropMargin, 1);
    const safeH = Math.max(actualH - cropMargin, 1);
    const maxLeft = actualW - safeW;
    const maxTop = actualH - safeH;

    // 3. Apply micro-crop + human-like modulation
    const processed = await sharp(resized.data)
        ...
        .modulate({
            brightness: 1.002 + (Math.random() * 0.006),
            saturation: 1.01 + (Math.random() * 0.02),
            hue: Math.round(Math.random() * 2 - 1) // Sharp hue requires an integer degree
        })
        .jpeg({ quality: 94, mozjpeg: true })
        .toBuffer();

    return { buffer: processed, ext: 'jpg', mimeType: 'image/jpeg' };
};
...
app.get('/api/health', (_req, res) => {
    try {
        ...
        // Read gallery stats from index (images are in nested subdirs, not flat root)
        let galleryCount = 0;
        let gallerySize = 0;
        try {
            if (fs.existsSync(GALLERY_INDEX)) {
                ...
                if (Array.isArray(entries)) {
                    ...
                    gallerySize = entries.reduce((sum, e) => sum + (e.fileSize || 0), 0);
                }
            }
        } catch { /* gallery index not yet created */ }
...
// ============================================
// SETTINGS — Configuration permanente (ex: Clé API sur Windows)
// ============================================
app.get('/api/settings', (_req, res) => {
    try {
        ...
        console.error('[Server] Erreur lecture settings.json:', err.message);
    }
    res.json({});
});
...
app.post('/api/settings', (req, res) => {
    try {
        ...
        const json = JSON.stringify(toWrite, null, 2);
        const tmpFile = path.join(SAVE_DIR, `.tmp.settings.${crypto.randomBytes(4).toString('hex')}.json`);
        // Atomic write for settings
        fs.writeFileSync(tmpFile, json, 'utf-8');
        fs.renameSync(tmpFile, SETTINGS_FILE);
        res.json({ ok: true });
    } catch (err) {
        console.error('[Server] Erreur ecriture settings.json:', err.message);
        res.status(500).json({ error: 'Erreur ecriture' });
    }
});
...
app.get('/api/gallery/:id/image', (req, res) => {
    ...
    const imgPath = path.resolve(GALLERY_DIR, safePath);

    // Prevent Path Traversal attacks effectively
    if (!imgPath.startsWith(path.resolve(GALLERY_DIR))) {
...
app.post('/api/gallery', largePayloadMiddleware, async (req, res) => {
    // Rate limit est géré globalement par apiLimiter désormais
    ...
    try {
        ...
        let ext = safeMime.includes('jpeg') ? 'jpg' : safeMime.includes('webp') ? 'webp' : 'png';
        let outputMime = safeMime;
        const id = `gal_${Date.now()}_${crypto.randomBytes(3).toString('hex')}`;

        let targetDir = GALLERY_DIR;
        let relativeDir = '';
        if (modelId && accountId) {
            // Use human-readable names, replace ONLY forbidden OS characters (<>:"/\|?*)
            const safeModelName = (modelName || modelId).replace(/[<>:"/\\|?*]/g, '_').trim() || 'models';
            const safeAccountHandle = (accountHandle || accountId).replace(/[<>:"/\\|?*]/g, '_').trim() || 'accounts';

            targetDir = path.resolve(GALLERY_DIR, safeModelName, safeAccountHandle);
            relativeDir = `${safeModelName}/${safeAccountHandle}/`;
        } else {
            targetDir = path.resolve(GALLERY_DIR, 'sandbox');
            relativeDir = 'sandbox/';
        }

        // Ultimate Traversal Security Check before creating any folder
        if (!targetDir.startsWith(path.resolve(GALLERY_DIR))) {
            return res.status(403).json({ error: 'Nom de modèle ou compte invalide (Path Traversal)' });
        }
        ...
        // Find highest existing numerical sequence like 004.jpg
        for (const f of files) {
        ...
        // --- STEALTH HYBRID SHARP PROCESSING ---
        // Strip data URI prefix if present (React canvas/file readers often include it)
        ...
        try {
            ...
            console.error('[Server] Stealth Sharp failed, falling back to raw:', sharpErr.message);
            finalBuffer = imgBuffer;
        }

        // Recalculate filename with the correct extension after Sharp processing
        const finalName = `${(highestNumber + 1).toString().padStart(3, '0')}.${ext}`;
        const finalRelativePath = relativeDir + finalName;

        fs.writeFileSync(path.join(targetDir, finalName), finalBuffer);
        // Update index
        const entries = readGalleryIndex();
        const entry = {
            id,
        ...
        while (entries.length > MAX_GALLERY) {
            ...
            try {
                if (removed && removed.filename) {
                    const safePath = path.normalize(removed.filename).replace(/^(\.\.(\/|\\|$))+/, '');
                    const removedPath = path.join(GALLERY_DIR, safePath);
                    if (fs.existsSync(removedPath)) fs.unlinkSync(removedPath);
                }
            } catch { /* ignore corrupted entries popping */ }
        }

        writeGalleryIndex(entries);
        console.log(`[Server] Gallery +1 — ${entries.length} images, ${finalRelativePath} (${(finalBuffer.length / 1024).toFixed(0)} KB)`);
...
// Intelligent orphan cleanup (handles macOS .DS_Store and Windows paths)
const removeEmptyParents = (filePath) => {
    try {
        let currentDir = path.dirname(filePath);
        const baseDir = path.resolve(GALLERY_DIR);
        // Ne remonte pas plus haut que GALLERY_DIR (safe cross-platform check)
        while (currentDir.length > baseDir.length && currentDir.startsWith(baseDir)) {
            // Un dossier est "vide" s'il ne contient que des fichiers cachés commençant par .
            const files = fs.readdirSync(currentDir).filter(f => !f.startsWith('.'));
            if (files.length === 0) {
                // Force deletion even if .DS_Store/Thumbs.db is inside
...
app.delete('/api/gallery/:id', (req, res) => {
    ...
    const safePath = path.normalize(removed.filename).replace(/^(\.\.(\/|\\|$))+/, '');
    const removedPath = path.resolve(GALLERY_DIR, safePath);

    if (removedPath.startsWith(path.resolve(GALLERY_DIR))) {
        try {
            fs.unlinkSync(removedPath);
            removeEmptyParents(removedPath);
        } catch { /* ignore */ }
    }
    writeGalleryIndex(entries);
...
app.delete('/api/gallery', (_req, res) => {
    try {
        ...
        for (const entry of entries) {
            const safePath = path.normalize(entry.filename).replace(/^(\.\.(\/|\\|$))+/, '');
            const targetPath = path.resolve(GALLERY_DIR, safePath);
            if (targetPath.startsWith(path.resolve(GALLERY_DIR))) {
                try { fs.unlinkSync(targetPath); } catch { /* ignore */ }
            }
        }

        // Clean all empty subdirectories recursively
        const cleanEmptyDirs = (dir) => {
            if (!fs.existsSync(dir)) return;
            const files = fs.readdirSync(dir);
            for (const f of files) {
                if (f.startsWith('.')) continue; // Ignorer les .DS_Store pour la récursion
                const fullPath = path.join(dir, f);
                if (fs.statSync(fullPath).isDirectory()) {
                    cleanEmptyDirs(fullPath);
                }
            }
            if (dir !== GALLERY_DIR && fs.readdirSync(dir).filter(f => !f.startsWith('.')).length === 0) {
                try { fs.rmSync(dir, { recursive: true, force: true }); } catch { /* ignore */ }
            }
        };
        cleanEmptyDirs(GALLERY_DIR);
        writeGalleryIndex([]);
